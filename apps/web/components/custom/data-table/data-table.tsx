...
import { DataTableRowSkeleton, DataTableRowMobileCardSkeleton } from './data-table-loading'
...
export function DataTable<TData, TValue>({
  ...
  const columns = React.useMemo(() => {
    ...
    if (onEdit || onDelete || customActions) {
      cols.push({
        id: 'actions',
        header: () => <div className="px-2 text-center">{dict.common.table.column_actions}</div>,
        cell: ({ row }) => {
          ...
          <div className="flex items-center justify-end gap-2">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" size="icon" onClick={onRefresh} disabled={isLoading}>
                    ...
                    <span className="sr-only">{dict.common.table.actions.refresh}</span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent>{dict.common.table.actions.refresh}</TooltipContent>
              </Tooltip>
            )}
            ...
            {(CreateForm || onAddClick) && (
              <Button onClick={onAddClick} className="flex-1 sm:flex-initial">
                ...
                {dict.common.table.actions.create}
              </Button>
            )}
          </div>
        </div>
